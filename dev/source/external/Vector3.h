...
namespace my {
    ...
    public:
        ...
        // BUG! (10-Jun-2019) CALLS FOR THE CVector3(x, y, z) CONSTRUCTOR WERE ENDING HERE!
        //CVector3(T value)
        //    : m_x(value),
        //    m_y(value),
        //    m_z(value),
        ...
        void operator=(const CVector2<T>& vector2)
        {
            m_x = vector2.m_x;
            m_y = vector2.m_y;
        ...
        // TODO: (??-???-2019) TO BE TESTED!
        void Rotate(T angleInDegrees, const CVector3<T>& axis)
        {
#if defined(_DEBUG)
            if (!IsValid())
                LOG_ERROR();
            ...
            T rotationMatrix[4][4] = { 0 },
                inputVectorData[4] = { m_x, m_y, m_z, 1.0 },
                outputVectorData[4] = { 0 };

            // (BEGIN OF) SET THE ROTATION MATRIX
            T axisSquaredLength = axis.Dot(axis),
                axisLength = sqrt(axisSquaredLength),
                angleInRadians = angleInDegrees * 3.1415926535897931 / 180.0,
                u = axis.x(),
                v = axis.y(),
                w = axis.z(),
                u2 = u * u,
                v2 = v * v,
                w2 = w * w,
                angleCos = (T)cos(angleInRadians),
                angleSin = (T)sin(angleInRadians);

            rotationMatrix[0][0] = (u2 + (v2 + w2) * angleCos) / axisSquaredLength;
            rotationMatrix[0][1] = (u * v * (1.0 - angleCos) - w * axisLength * angleSin) / axisSquaredLength;
            rotationMatrix[0][2] = (u * w * (1.0 - angleCos) + v * axisLength * angleSin) / axisSquaredLength;
            rotationMatrix[0][3] = 0.0;
            rotationMatrix[1][0] = (u * v * (1.0 - angleCos) + w * axisLength * angleSin) / axisSquaredLength;
            rotationMatrix[1][1] = (v2 + (u2 + w2) * angleCos) / axisSquaredLength;
            rotationMatrix[1][2] = (v * w * (1.0 - angleCos) - u * axisLength * angleSin) / axisSquaredLength;
            rotationMatrix[1][3] = 0.0;
            rotationMatrix[2][0] = (u * w * (1.0 - angleCos) - v * axisLength * angleSin) / axisSquaredLength;
            rotationMatrix[2][1] = (v * w * (1.0 - angleCos) + u * axisLength * angleSin) / axisSquaredLength;
            rotationMatrix[2][2] = (w2 + (u2 + v2) * angleCos) / axisSquaredLength;
            rotationMatrix[2][3] = 0;
            rotationMatrix[3][0] = 0;
            rotationMatrix[3][1] = 0;
            rotationMatrix[3][2] = 0;
            rotationMatrix[3][3] = 1;
            // (END OF) SET THE ROTATION MATRIX

            for (int i = 0; i != 4; ++i)
            {
                outputVectorData[i] = 0;

                for (int k = 0; k < 4; ++k)
                    outputVectorData[i] += rotationMatrix[i][k] * inputVectorData[k];
            }

            m_x = outputVectorData[0];
            m_y = outputVectorData[1];
            m_z = outputVectorData[2];
        }
