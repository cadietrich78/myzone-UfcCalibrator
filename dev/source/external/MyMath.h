...
#define COS_80 0.17364817766693034885171662676931
...
namespace MyMath
{
	...
	*/
    ...
    // [0, INF) https://en.wikipedia.org/wiki/Bhattacharyya_distance
    template <typename T>
    T BhattacharyyaDistance(T pMean, T pStddev, T qMean, T qStddev);

    template <>
    inline double BhattacharyyaDistance(double pMean, double pStddev, double qMean, double qStddev)
    {
        double pStddevSqrd = pStddev * pStddev,
            qStddevSqrd = qStddev * qStddev,
            meanDiff = pMean - qMean;

        return 0.25 * log(0.25 * (pStddevSqrd / qStddevSqrd + qStddevSqrd / pStddevSqrd + 2.0)) + 0.25 * (meanDiff * meanDiff / (pStddevSqrd + qStddevSqrd));
    }

    template <>
    inline float BhattacharyyaDistance(float pMean, float pStddev, float qMean, float qStddev)
    {
        float pStddevSqrd = pStddev * pStddev,
            qStddevSqrd = qStddev * qStddev,
            meanDiff = pMean - qMean;

        return 0.25f * logf(0.25f * (pStddevSqrd / qStddevSqrd + qStddevSqrd / pStddevSqrd + 2.0f)) + 0.25f * (meanDiff * meanDiff / (pStddevSqrd + qStddevSqrd));
    }
    ...
    */
    ...
    bool Derivative(int n, const ValueType *y, ValueType *dy)
    {
        ...
        // BUG: (06-Jan-2019) THE LAST VALUE CANNOT BE 0! IT WILL MESS UP WITH ANY DERIVATIVE TEST!
        //dy[n - 1] = 0;
        dy[n - 1] = d;
    ...
    */
    ...
    // Fast inverse square root. The following code is the fast inverse square root implementation from Quake III Arena, stripped of C preprocessor directives, but including the exact original comment text:
...
#define MEDIAN_FILTER_SWAP(a, b) if (window[a] > window[b]) std::swap(window[a], window[b]);
    ...
    */
    inline void MirrorPointThroughPlane(double planeCoeff[4], const double i[3], double o[3])
    {
        double k = (-planeCoeff[0] * i[0] - planeCoeff[1] * i[1] - planeCoeff[2] * i[2] - planeCoeff[3]) / (planeCoeff[0] * planeCoeff[0] + planeCoeff[1] * planeCoeff[1] + planeCoeff[2] * planeCoeff[2]);

        double x2 = planeCoeff[0] * k + i[0],
            y2 = planeCoeff[1] * k + i[1],
            z2 = planeCoeff[2] * k + i[2];
    ...
    */
    ...
    // DEPRECATED: (07-Oct-2020)
    //template <typename T>
    //double PointToSegment(T p, T s0, T s1)
    //{
    //    T v = s1 - s0,
    //        w = p - s0;

    //    HEALTH_CHECK(IsZero(v.Length()), my::Null<double>());
 ...
 //       HEALTH_CHECK(!segmentEnd, 0);
    ...
    template <typename T>
    double StdDev(T mean, T a[], int n)
    {
        double s = 0,
            p = 0,
            variance = 0;
