...
static size_t ReadCallbackForCurl(void *ptr, size_t size, size_t nmemb, void *stream)
{
...
std::string my::file::PutToUrl(std::string url, std::string fileName, std::string user, std::string password)
{
    ...
    curl_easy_setopt(curl, CURLOPT_READFUNCTION, ReadCallbackForCurl);
...
// FILE TO FTP (https://curl.haxx.se/libcurl/c/ftpupload.html)
// timeout: the maximum time in seconds that you allow the transfer operation to take.
bool my::file::UploadToFtp(std::string fileName, std::string url, std::string user, std::string password, my::int32 timeout)
{
#if !defined( MY_NO_CURL )
    ...
    std::string fileNameWithoutPath = my::GetFileName(fileName);

    if (my::IsNull(fileNameWithoutPath))
    {
        fclose(fileStream);
    ...
    curl_easy_setopt(curl, CURLOPT_URL, (my::AddTrailingSlash(url) + fileNameWithoutPath).c_str());
    ...
    if (response != CURLE_OK)
        LOG_MESSAGE(std::string("curl_easy_perform() failed: ") + curl_easy_strerror(response));
...
struct FileHandleForWriteCallback {
    const char *filename;
    FILE *stream;
};

// https://curl.haxx.se/libcurl/c/ftpget.html
static size_t WriteCallbackForCurl(void *buffer, size_t size, size_t nmemb, void *stream)
{
    struct FileHandleForWriteCallback *fileHandle = (struct FileHandleForWriteCallback *)stream;

    if (!fileHandle->stream)
    {
        fileHandle->stream = fopen(fileHandle->filename, "wb");

        if (!fileHandle->stream)
            return -1;
    }

    return fwrite(buffer, size, nmemb, fileHandle->stream);
}

// FILE TO FTP (https://curl.haxx.se/libcurl/c/ftpget.html)
// timeout: the maximum time in seconds that you allow the transfer operation to take.
bool my::file::GetFromFtp(std::string fileName, std::string url, std::string user, std::string password, my::int32 timeout)
{
#if !defined( MY_NO_CURL )
    struct FileHandleForWriteCallback ftpfile = {
        fileName.c_str(),
        NULL
    };
